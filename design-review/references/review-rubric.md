...
对 `design.md` 评审的权威检查清单。Self-contained —— do not depend on other skills' files。Apply the dimensions，为每个发现标记严重性，然后计算裁决。

## Contents

- Canonical section set（完整的 design.md 包含什么）
...
由 brainstorming skill 产出的 design.md 遵循此结构。Use it as the reference for completeness (D1) and conformance (D3)。Headings are in English；section content may be in any language。

| Section | Required? | Purpose |
|---------|-----------|---------|
| Summary | Yes | 一段话：正在构建什么以及为什么 |
| Goals | Yes | 主要成果 + 可衡量的成功指标 |
| Primary Users / Roles | Yes | 谁使用这个以及他们的关键目标 |
| Non-Goals | Yes | 明确超出范围的内容 |
| Context | Yes | 现有行为、约束、相关模块 |
| Discovery (Key Discoveries / Scope Decisions) | Recommended for non-trivial/ambiguous requests | 测试的假设、浮现的盲点、什么在范围内/外以及为什么 |
| Decision Record (Options Considered + Decision & Rationale) | Yes | 权衡的备选方案以及为什么选择这个（或为什么只有一个可行） |
| Proposed Solution → Architecture | Yes | 关键构建块和关系 |
| Proposed Solution → Components | Yes | 具有职责和接口的组件 |
| Proposed Solution → Data Flow | Yes | 主要路径的逐步流程 |
| Error Handling | Yes | 主要失败模式以及如何处理 |
| Testing | Yes | 关键测试用例以及在哪里运行 |
| Open Questions | Yes | Only 已浮现的未解决问题；or "None" |

A missing **required** section is at least a Major finding，当缺失的章节对实现至关重要时（e.g., no Components, no Data Flow, no Error Handling）则是 Blocker。

## The 7 dimensions

For each finding，记录：location（`design.md §section` and/or project file）、evidence、recommendation、severity、and the dimension it belongs to。
...
Pick the checklists relevant to the design's domain (below) and surface gaps the design does not address。Frame each as "值得检查"，and verify Open Questions captured the genuinely open unknowns rather than leaving them implicit。
...
- A component whose only justification is "我们将来可能需要它"。
