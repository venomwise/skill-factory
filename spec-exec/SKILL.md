...
派发能力由首个包的实际派发验证。IF 首次派发失败，THEN 记录尝试过的机制和具体失败原因，
该包和后续包回到主线程顺序执行，不阻塞流程；未经实际尝试不得以「运行环境不支持」为由跳过派发。
...
6. **选择执行模式。**
   ...
   - 执行模式默认**主线程顺序执行**。IF 剩余未完成任务可拆出至少两个满足分包条件的工作包，
     THEN 允许一次性询问：`A. 主线程顺序执行（默认） B. 工作包派发模式`。只有一个可用包时
     不提供该选项，不阻塞流程。不要在此自评运行环境能否启动实现子代理：工具清单里没有名字
     显眼的代理工具不等于没有可用机制，派发能力由用户选择 B 后的首次实际派发验证。
...
- 工作包模式必须经用户确认分包方案后才派发；首次实际派发失败时回到主线程模式，并记录
  尝试过的机制和失败原因，不阻塞。
