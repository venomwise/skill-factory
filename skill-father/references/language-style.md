...
本节约束 skill 生成或修改的项目计划、设计、评审和其他文档，不只约束 skill 自身与用户
交流时的语气。产出型 skill 必须把核心规则写入自己的工作流，不能假设运行环境还安装了
另一个文风 skill。

写完后重新读取实际文件，并按技术文档场景做轻量检查：

1. 优先写清主体、动作、条件、对象和结果，不用抽象名词代替实际关系。
2. 处理逐词翻译、商业黑话、流程隐喻、模板化开场和空泛总结，但不使用机械禁词表。
3. 保留数字、事实、责任主体、决策、风险、状态、ID、路径、命令、代码、引用和技术术语。
4. 不为了自然表达补充原文没有的依据、效果、方案或责任关系。
5. 不降低正式技术文档的准确性，也不把说明文改成聊天体或宣传文。
6. 修改语言后重新检查结构、状态、追踪关系和格式；发现含义变化时回退并缩小改写范围。

不同产物还应增加自己的具体检查：项目计划检查用户或业务结果，设计检查 AC 和契约，
评审检查 Issue、Evidence、Severity 和 Recommendation，修改型 skill 只检查本次改动及相邻
段落，除非用户明确要求清理全文。
