...
var (
	// ErrNoAPIKey is returned when no valid Exa API key is configured
	ErrNoAPIKey = errors.New("no valid Exa API key configured")
)

// ExecuteSearch executes a search with failover across profiles
func ExecuteSearch(ctx context.Context, cfg config.ResolvedProviderConfig, req SearchRequest) (*Response, []Attempt, error) {
	if len(cfg.Profiles) == 0 {
		return nil, nil, ErrNoAPIKey
	}

	timeout := time.Duration(cfg.Timeout) * time.Second
	client := New(cfg.Profiles, cfg.BaseURL, timeout)

	return client.Search(ctx, req)
}

// ExecuteSimilar executes a find similar request with failover across profiles
func ExecuteSimilar(ctx context.Context, cfg config.ResolvedProviderConfig, req SimilarRequest) (*Response, []Attempt, error) {
	if len(cfg.Profiles) == 0 {
		return nil, nil, ErrNoAPIKey
	}
	...
	return client.FindSimilar(ctx, req)
}
...
func BuildRetryGuidance(classification string, attemptCount int) string {
	...
	case "auth_failure":
		return "Authentication failed. Check your Exa API key configuration."
	case "rate_limit":
	...
	case "server_error":
		return "Exa API server error. The service may be temporarily unavailable."
	case "retryable":
