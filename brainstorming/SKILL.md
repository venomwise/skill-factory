...
5. Converge and propose. Admission gate: do not enter this step until step 4's exit condition is actually met — the user can state what they want, why, and what they explicitly do not want, and key constraints and success criteria are known or recorded as assumptions. If you have not yet asked the user anything in this session, you have not met the gate; go back to step 4. A request being technically detailed does not satisfy the gate, because technical detail says nothing about the user's intent, priorities, or trade-off preferences. Once the gate is met, summarize what brainstorming revealed: the refined problem statement, request-fit findings, challenged assumptions, discovered blind spots, and trimmed scope. Then propose 1-3 approaches with trade-offs. Lead with your recommendation. If only one approach is viable, explain why alternatives were ruled out. Ask the user to confirm the problem summary and select an approach before continuing.
