...
4. Brainstorm with the user through iterative rounds. First diagnose the fuzziness level of the request (see the guide for criteria), then apply matched techniques:
   ...
   Continue asking until the exit condition is met. Exit condition: the user can clearly state what they want, why they want it, and what they explicitly do not want, and key constraints and success criteria are known or explicitly recorded as assumptions. Before leaving this step, verify: can you summarize the user's intent, priorities, and constraints in 2-3 sentences using their own words? If not, ask more questions.
